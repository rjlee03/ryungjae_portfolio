...
namespace demos
{
    ...
    void D01HelloQuad::Draw() const
    {
        ...
        const float mouseX = (window::input::MouseWindowX / static_cast<float>(window::WindowWidth))  * 2.0f - 1.0f;
        const float mouseY = 1.0f - (window::input::MouseWindowY / static_cast<float>(window::WindowHeight)) * 2.0f;
        shader.SendUniform("uMousePos", glm::vec2(mouseX, mouseY));
        quadMesh.Use();
