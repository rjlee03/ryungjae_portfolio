...
namespace demos
{
    ...
    {
        const auto& writeDepthShader = shaders[Shaders::WriteDepth];
        writeDepthShader.Use();
        writeDepthShader.SendUniform(Uniforms::Projection, lightProjectionMatrix);
        writeDepthShader.SendUniform(Uniforms::ViewMatrix, lightCamera.ViewMatrix());
        writeDepthShader.SendUniform(Uniforms::NearDistance, lightNear);
        writeDepthShader.SendUniform(Uniforms::FarDistance, lightFar);
        ...
        drawSceneObjects(writeDepthShader, drawBackFacesForRecordDepthPass ? GL_FRONT : GL_BACK);
        GL::CullFace(GL_BACK);
    ...
    void D05ShadowMapping::drawLightFrustum() const
    {
        ...
        {
            const auto& fillShader = shaders[Shaders::Fill];
            fillShader.Use();
            fillShader.SendUniform(Uniforms::Projection, projectionMatrix);
            fillShader.SendUniform(Uniforms::ViewMatrix, camera.ViewMatrix());
            fillShader.SendUniform(Uniforms::Diffuse, glm::vec3(1.0f));
            const glm::mat4 ModelMatrix = lightCamera.ToWorldMatrix() * glm::inverse(lightProjectionMatrix);
            fillShader.SendUniform(Uniforms::ModelMatrix, ModelMatrix);
    ...
    void D05ShadowMapping::drawDepthTexture() const
    {
        if (shouldDrawDepthTexture)
        {
            ...
            const auto& viewDepthShader = shaders[Shaders::ViewDepth];
            viewDepthShader.Use();
            viewDepthShader.SendUniform(Uniforms::ShadowMap, 0);
